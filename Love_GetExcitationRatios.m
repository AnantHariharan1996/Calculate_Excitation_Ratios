...
for currN = [0:1:MaxN]
    ...
    fname=[ 'atl2a_Love_TUc_br' num2str(currN) '.dms'];
    tucinfo=load(fname);
    Tlist=tucinfo(:,1);
    Clist=tucinfo(:,3);
    ...
    for period = Periodlist
        ...
        % Load eigenfunction
        Eigfname=['T_B' num2str(currN) '_' num2str(period) 's'];
...
for period = Periodlist
    ...
    dlmwrite(RawExcitationFname,PeriodStruc(periodcounter).RawExcitation_Mat,'delimiter','\t','precision','%.9f')

    % Output Excitation Ratios as text file  
    dlmwrite(ExcitationRatioFname,ExcitationRatio_Mat,'delimiter','\t','precision','%.9f')
